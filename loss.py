...
class Losses:
    ...
    def _bi_weight(self):
        frequency=defaultdict(lambda:torch.tensor(0.).to(self.device))
    ...
    def _total_loss(self):
        ...
        metrics['segm'] += segmentation_loss * self.seg_mask.size(0)
        metrics['discr'] += discriminative_loss * self.seg_mask.size(0)
        metrics['tot'] += total_loss * self.seg_mask.size(0)
